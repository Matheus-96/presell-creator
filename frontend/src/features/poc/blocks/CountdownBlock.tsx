...
export function CountdownBlockComponent({ block }: Props) {
  const { variant = "block", minutes, label, textColor, backgroundColor } = block;
  const { mm, ss } = useCountdown(minutes);
    ...
    return (
      <div
        style={{
          ...
          gap: "var(--p-space-3)",
          padding: "var(--p-space-3) var(--p-block-padding-x)",
          ...(backgroundColor ? { backgroundColor } : { backgroundColor: "#c0392b" }),
